...
def get_lora_compatible_modules(model, verbose=False):
    lora_compatible_modules_names = []
    for n, m in model.named_modules():
        if isinstance(m, LORA_COMPATIBLE_LAYERS):
            lora_compatible_modules_names.append(n)

        if verbose:
            if isinstance(m, LORA_COMPATIBLE_LAYERS):
                print("LORA_COMPATIBLE:", n, type(m))
            else:  
                print("NOT LORA_COMPATIBLE:", n, type(m))
    
    return lora_compatible_modules_names
...
class Trainer:
    ...
    def train(self) :
        ...
        # I. Intial evaluation
        if self.t_args.logging.initial_eval:
            if self.accelerator.is_main_process:
    ...
    def train_vae(self):
        ...
        # II. Training loop
        with tqdm.tqdm(total=self.t_args.optim.n_epochs * len(self.dataloader_injection), disable=not self.accelerator.is_main_process) as pbar:
            for epoch in range(self.t_args.optim.n_epochs):
                pbar.set_description("/!\ DEBUGING /!\ " if self.debug else f"   epoch [{epoch}/{self.t_args.optim.n_epochs}]")
                train_epoch_start = time.time()

                # 2. Iterate over the dataloader and update NerfVAE
                for iter_nb, batch_injection in enumerate(self.dataloader_injection):
                    self.injection_iter(batch_injection)
    ...
    def eval(
        ...
        if consistency_metrics:
            eval_dict['consistency_metrics/'] = self.evaluator.get_consistency_metrics(
                _nerf_vae.vae,
                _nerf_vae.latent_nerfs[:self.config.eval.metrics.n_scenes],
                _nerf_vae.latent_renderer,
                self.normalizer,
                batch_size=self.t_args.consistency.batch_size,
                device=self.accelerator.device
...
def init_models(config, n_scenes) : 
    ...
    if config.lora.apply:
        vae.requires_grad_(False)
        lora_compatible_modules = get_lora_compatible_modules(vae)
        lora_config = peft.LoraConfig(
            r=config.lora.rank,
            lora_alpha=config.lora.alpha,
            init_lora_weights="gaussian",
            target_modules=lora_compatible_modules
        )
        vae = peft.get_peft_model(vae, lora_config)
        print("Lora ENABLED.")
